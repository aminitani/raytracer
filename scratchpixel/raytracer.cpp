...
template<typename T>
void thrender(int start, int end, unsigned width, T invWidth, T invHeight, T angle, T aspectratio, const std::vector<Sphere<T> *> &spheres, Vec3<T> *pixel) {
	// Trace rays
...
void render(const std::vector<Sphere<T> *> &spheres)
{
	...
	unsigned int threadCnt = 20;

	int chunkSize = height / threadCnt;

	for(unsigned thread = 0; thread < threadCnt-1; thread++)
	{
		int start = thread * height / threadCnt;
		int end = start + chunkSize;
		threads.push_back(std::thread(thrender<float>, start, end, width, invWidth, invHeight, angle, aspectratio, spheres, pixel));
		// for(int i = 0; i < chunkSize * width;
		pixel += chunkSize * width;
	}
	threads.push_back(std::thread(thrender<float>, (threadCnt-1) * height / threadCnt, height, width, invWidth, invHeight, angle, aspectratio, spheres, pixel));
